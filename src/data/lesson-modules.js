...
export const MODULE_TIME_LABEL = "18â€“20 min";
...
If there is no exact tag match, ask the **AI tutor**: â€œGive me a 5-minute drill on ${lesson.title} with a rubric.â€`;
}
...
If you can narrate all five of these, you are ahead of most Python screens.`,
  ...
  "py-b4": {
    durationLabel: MODULE_TIME_LABEL,
    outcomes: [
      "Explain **why** `dict` and `set` lookups are O(1) average â€” in terms of hashing, not magic.",
      "Pick the right container fast: `dict` for keyâ†’value, `set` for membership, `Counter` / `defaultdict` for the common ETL cases.",
      "State the **hashability contract** (`__hash__` + `__eq__`) and predict which types can be keys.",
      "Use dict/set **operators** fluently: `|`, `&`, `-`, `^`, `|=`, `{**a, **b}` â€” and spot their complexity.",
...
**Custom classes:** if you override \`__eq__\`, Python sets \`__hash__ = None\` unless you also define \`__hash__\`. Equal objects **must** hash to the same value â€” otherwise dict/set silently loses them.
...
- **Forgetting \`else:\`** â€” silent fall-through is the cause of half of all "function returned None" bugs. If every branch returns, prefer **early-return**: \`if cond: return x; return y\`.
- **\`case [c]:\` against a string** â€” does not match. Strings are intentionally excluded from sequence patterns.
- **OR patterns binding inconsistent names** â€” \`case [x] | (x, y)\` is a SyntaxError because the two alternatives bind different names.
- **Putting \`case _:\` first** â€” every subsequent case is dead code. The linter won't always catch this.
...
"A Python signature has five slots in fixed order: positional-only (before \`/\`), positional-or-keyword, \`*args\`, keyword-only (after \`*\` or \`*args\`), \`**kwargs\`. The bare \`*\` is how I make boolean flags keyword-only at the call site so the code reads. Defaults are evaluated once at definition time, so I use \`None\` as a sentinel for any mutable default. Decorators forward with \`(*args, **kwargs)\` plus \`functools.wraps\` to preserve introspection. The \`TypeError\`s â€” \`missing\`, \`unexpected\`, \`multiple values\` â€” all map back to the same five binding rules."`,
